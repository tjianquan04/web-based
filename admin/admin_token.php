...
$_db->query('DELETE FROM admin_token WHERE expire < NOW()');

$id = req('id');

// TODO: (2) Is token id valid?
if (!is_exists($id, 'admin_token', 'id')) {
...
if (is_post()) {
    ...
    if (!$_err) {
        ...
        $stm = $_db->prepare('
            UPDATE admin
            SET password = SHA1(?)
            WHERE admin_id = ( SELECT admin_id FROM admin_token WHERE id = ?);

            DELETE FROM admin_token WHERE id = ?;
        ');
